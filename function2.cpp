...
veryLong veryLong::operator/(veryLong n1)
{
    ...
    if (!binary)
    {
        if (((sign=='+')&&(n1.sign=='+'))||((sign=='-')&&(n1.sign=='-')))
        {
            n.number = divv(&number,&n1.number,&remainder);
            n.sign='+';
        }
        else
        {
    ...
    else
    {
        ...
        n.number = divvb(&number,&n1.number,length,n1.length,&n.length,&remainder,&lenOst);
        n.binary=binary;
        turnb(&number,length);
        turnb(&n1.number,n1.length);
...
{
    int i;
    (*this).number=new unsigned char [strlen(str)];
...
veryLong veryLong::operator=(veryLong n1)
{
    ...
    (*this).number = new unsigned char[n1.length];
...
veryLong:: veryLong(const veryLong & n)
{
    ...
    (*this).number= new unsigned char [n.length];
